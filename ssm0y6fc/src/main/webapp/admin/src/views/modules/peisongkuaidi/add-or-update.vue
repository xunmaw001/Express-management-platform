<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="配送类型" prop="peisongleixing">
          <el-select :disabled="ro.peisongleixing" v-model="ruleForm.peisongleixing" placeholder="请选择配送类型">
            <el-option
                v-for="(item,index) in peisongleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="配送类型" prop="peisongleixing">
	      <el-input v-model="ruleForm.peisongleixing"
                placeholder="配送类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="用户备注" prop="yonghubeizhu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="用户备注"
                  v-model="ruleForm.yonghubeizhu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yonghubeizhu" label="用户备注" prop="yonghubeizhu">
                    <span>{{ruleForm.yonghubeizhu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  data() {
	...
	userid : false,
      ...
      rules: {
          ...
          zhidingweizhi: [
                { required: true, message: '指定位置不能为空', trigger: 'blur' },
          ],
          peisongfei: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `peisongkuaidi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.peisongkuaidiCrossAddOrUpdateFlag = false;
						 this.parent.search();
