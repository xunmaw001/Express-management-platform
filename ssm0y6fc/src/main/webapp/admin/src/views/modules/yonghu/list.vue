<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yonghu','添加快递') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="kuaidixinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'添加快递':'' }}</el-button>
                <el-button v-if="isAuth('yonghu','添加快递') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="kuaidixinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'添加快递':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yonghu','添加快递') && contents.tableBtnIcon == 0" type="success" size="mini" @click="kuaidixinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'添加快递':'' }}</el-button>
    ...
    <kuaidixinxi-cross-add-or-update v-if="kuaidixinxiCrossAddOrUpdateFlag" :parent="this" ref="kuaidixinxiCrossaddOrUpdate"></kuaidixinxi-cross-add-or-update>
...
import kuaidixinxiCrossAddOrUpdate from "../kuaidixinxi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      kuaidixinxiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    kuaidixinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.kuaidixinxiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.kuaidixinxiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.kuaidixinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
